...
class interfaceController extends baseController {
    ...
    async listByCat(ctx) {
        ...
        try {
            ...
            let menuList = [], _menuList = [], menuTree = [];
            let catItem = {}, cats = [];
            // 01 查询所有菜单项并构造树形结构(不包含接口)
            menuList = await this.catModel.list(project_id);
            if(menuList) menuTree = yapi.commons.buildTree(menuList);
            // 02 解树结构获得平铺list
            if(menuTree) _menuList = yapi.commons.tree2list(menuTree);
            // 03 找到要查询的catid及其所有下级cat
             if(_menuList) {
                 catItem = _menuList.filter(menu => {
                     return Number(menu._id) === Number(catid)
                 })[0];
                 cats = yapi.commons.getChildren(catItem);
             }

            let project = await this.projectModel.getBaseInfo(catItem.project_id);
            if (project.project_type === 'private') {
                if ((await this.checkAuth(project._id, 'project', 'view')) !== true) {
                    return (ctx.body = yapi.commons.resReturn(null, 406, '没有权限'));
                }
    ...
    * */
    async listMenu(ctx) {
        ...
        try {
           ...
           // ctx.body = yapi.commons.resReturn(yapi.commons.buildTree(newResult));
        } catch (err) {
            ctx.body = yapi.commons.resReturn(null, 402, err.message);
        }
    }

    async listMenuTree(ctx) {
        let project_id = ctx.params.project_id;
        if (!project_id) {
            return (ctx.body = yapi.commons.resReturn(null, 400, '项目id不能为空'));
        }
    ...
    async addCat(ctx) {
        try {
            ...
            let result = await this.catModel.save(data);
    ...
    delCat(ctx) {
        try {
            let result = this.delCatRecur(ctx.request.body.catid);
            ctx.body = result;
        } catch (e) {
            ctx.body = yapi.commons.resReturn(null, 400, e.message);
        }
    ...
    * */
    async delCatRecur(catid) {
        ...
        // 递归查询所有子菜单并删除
        let children = await this.catModel.listChilden(id);
        if(children) {
            children.forEach(child => {
                this.delCatRecur(child._id);
            });
        }

        return yapi.commons.resReturn(r);
    }
     ...
     * 对于导入的目录和接口，重建父子关系
     */
    async rebuildCat(ctx){
        try {
            let cats = await this.catModel.rebuildCatTree();
            return (ctx.body = yapi.commons.resReturn(cats));
        } catch (e){
            yapi.commons.resReturn(null, 400, e.message);
        }
    }
