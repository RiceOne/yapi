...
class InterfaceList extends Component {
    ...
    handleChangeInterfaceCat = (desc, name) => {
        ...
        axios.post('/api/interface/up_cat', params).then(async res => {
            ...
            await this.props.fetchInterfaceMenuTree(project_id);
    ...
    handleAddInterface = data => {
        ...
        axios.post('/api/interface/add', data).then(res => {
            ...
            this.props.fetchInterfaceMenuTree(data.project_id);
        });
    };
    ...
    changeInterfaceCat = async (id, catid) => {
        ...
        if (result.data.errcode === 0) {
            ...
            this.handleRequest(this.props);
            this.props.fetchInterfaceMenuTree(this.props.curProject._id);
        } else {
     ...
     * */
        ...
        // 递归遍历树，构造<id, name>键值对
    cacheAllCat = (cats, uri) => {
        let catMap = new Map();
        if (cats) {
            cats.forEach(cat => {
                let _uri = uri + "/" + cat.name;
                catMap.set(cat._id, _uri)
                if (cat.children) {
                    const childCatMap = this.cacheAllCat(cat.children, _uri);
                    for (let [k, v] of childCatMap) {
                        catMap.set(k, v)
                    }
                }
            })
    ...
    getChildrenCats = (cat) => {
        let all_child_cats = [];
        all_child_cats.push(cat.id)
        if (cat.children) {
            cat.children.forEach(child => {
                all_child_cats = all_child_cats.concat(this.getChildrenCats(child))
            })
        }
    ...
    render() {
        ...
        const catMap = this.cacheAllCat(this.props.catList, "");
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (item, record) => {
                    return (
                        <div>{catMap.get(item) + ''}</div>
                        /* <Select
                           value={catMap.get(item) + ''}
